...
class ItemService:
    ...
    def get_item_base_info(self, request: ItemRequest) -> Dict[str, Any]:
        ...
        if not item:
            raise MetadataError(f"Item {request.itemId} not found in collection {collection}")
        ...
        if media_type == 1 and "metadata" in item and "segment_info" in item["metadata"]:
    ...
    def _process_metadata(self, metadata: Dict[str, Any]) -> List[Tuple[str, List[str]]]:
