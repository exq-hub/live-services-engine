...
class SearchService:
    ...
    def __init__(self, model_manager, index_repository, metadata_repository):
        ...
        self.strategies: Dict[str, SearchStrategy] = {
            "clip": CLIPSearchStrategy(model_manager, index_repository, metadata_repository),
            "caption": CaptionSearchStrategy(model_manager, index_repository, metadata_repository),
            "rf": RFSearchImpl(model_manager, index_repository, metadata_repository),
            "aggregate": AggregateSearchStrategy(model_manager, index_repository, metadata_repository),
        }
    ...
    async def search_text(self, strategy_name: str, request: TextSearchRequest) -> Dict:
        ...
        try:
            ...
            raise SearchError(f"Text search failed with strategy {strategy_name}: {e}", {
                "strategy": strategy_name,
                "collection": request.session_info.collection,
                "text": request.text
            })
