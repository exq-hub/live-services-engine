...
class MetadataRepository:
    ...
    def load_related_items(self, collection: str, related_items_file: str) -> Dict[str, List[int]]:
        ...
        try:
            ...
            if not related_path.exists():
                raise MetadataError(f"Related items file not found: {related_items_file}")
            ...
            raise MetadataError(f"Invalid JSON in related items file {related_items_file}: {e}")
        except Exception as e:
            raise MetadataError(f"Failed to load related items from {related_items_file}: {e}")
