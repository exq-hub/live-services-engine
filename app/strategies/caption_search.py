...
class CaptionSearchStrategy(TextSearchStrategy):
    ...
    async def search(self, 
                    ...
                    seen: List[int],
                    excluded: List[int],
                    filters: Optional[ActiveFilters] = None) -> List[int]:
        ...
        try:
            ...
            raise SearchError(f"Caption search failed: {e}", {"collection": collection, "text": text})
    ...
    async def _search_with_expansion(self, collection: str, text_features: np.ndarray,
                                   n: int, seen_set: set, excluded_set: set,
                                   filters: Optional[ActiveFilters]) -> List[int]:
        ...
        item_to_datapoint = self.metadata_repo.create_item_to_datapoint_mapping(collection)
        
        while True:
            last = active_n >= total_items
            
            # Search caption index
            _, indices = self.index_repo.search_caption(collection, text_features, active_n)
            
            # Map caption indices to shot indices
            caption_ids = [
                caption_shot_ids_list[idx] for idx in indices[0].tolist()
            ]
            base_shots = [
                shot_overlap_mapper[cap_id][0] for cap_id in caption_ids
            ]
