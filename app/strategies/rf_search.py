...
class RFSearchStrategy(RFSearchStrategy):
    ...
    def __init__(self, model_manager, index_repository, metadata_repository):
        ...
        self.clip_search = CLIPSearchStrategy(model_manager, index_repository, metadata_repository)
    ...
    async def search(self, 
                    ...
                    query: Optional[str] = None) -> List[int]:
        ...
        try:
            ...
            labels = np.concatenate(([1.0] * len(pos_samples), [-1.0] * len(neg_samples)))
            ...
            raise SearchError(f"RF search failed: {e}", {
                "collection": collection,
                "pos_count": len(pos),
                "neg_count": len(neg)
            })
    
    async def _prepare_positive_samples(self, collection: str, pos: List[int], 
                                      query: Optional[str], seen: List[int],
                                      excluded: List[int], filters: Optional[ActiveFilters]) -> np.ndarray:
    ...
    async def _search_with_expansion(self, collection: str, hyperplane: np.ndarray,
