...
class AggregateSearchStrategy(TextSearchStrategy):
    ...
    def __init__(self, model_manager, index_repository, metadata_repository):
        ...
        self.caption_search = CaptionSearchStrategy(model_manager, index_repository, metadata_repository)
    ...
    async def search(self, 
        ...
        try:
            ...
            raise SearchError(f"Aggregate search failed: {e}", {"collection": collection, "text": text})
    
    def _calculate_rrf_scores(self, clip_results: List[int], caption_results: List[int]) -> dict:
