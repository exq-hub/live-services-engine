...
def timer_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        logger.info(f"{current_time} - {func.__name__} executed in {time_taken:.3f} seconds")
        return result
    return wrapper
...
class ModelManager:
    ...
    def initialize_collection_models(self, collection: str, collection_config, metadata_repo: MetadataRepository):
        """Initialize collection-specific models and mappers."""
        if (collection_config.caption_index and 
            collection_config.caption_shot_mapping_file and 
            collection_config.segment_duration):
            ...
            try:
                ...
                logger.warning(f"Failed to initialize shot overlap mapper for {collection}: {e}")
    
    @timer_decorator
    def _init_shot_overlap_mapper(self, collection: str, collection_config, metadata_repo: MetadataRepository):
...
class ApplicationContainer:
    ...
    def initialize(self):
        ...
        for collection in config.collections:
            ...
            metadata_repo.load_related_items(collection, collection_config.related_items_file)
            
            # Load indices
            index_repo.load_clip_index(collection, collection_config.clip_index)
            
            if collection_config.caption_index:
                index_repo.load_caption_index(collection, collection_config.caption_index)
            
            # Load PCA data or embeddings
            if collection_config.pca_model and collection_config.std_scaler and collection_config.pca_embeddings_file:
                ...
                index_repo.set_embeddings_zarr_path(collection, collection_config.embeddings_file)
            
            # Initialize collection-specific models
            model_manager.initialize_collection_models(collection, collection_config, metadata_repo)
    ...
    def _display_system_info(self):
        ...
        console.print(Panel(system_name, title="Exquisitor", border_style="bold yellow"))
